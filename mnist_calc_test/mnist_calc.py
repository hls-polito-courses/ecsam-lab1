...
class TouchCalc:
    def __init__(self, window_title='MNIST DrawCalc', width=1400, height=500):
        ...
        self._op1, self._op1_prob, self._op1_delay = None, None, None
        self._op2, self._op2_prob, self._op2_delay = None, None, None
        self._answer = None
    ...
    def _draw_results(self):
        ...
        self._op1_label.label = '{:d} ({:.2f}% prob., {:.2f}s)'.format(self._op1, self._op1_prob * 100, self._op1_delay) if self._op1 is not None else 'No digit detected.'
        self._op2_label.label = '{:d} ({:.2f}% prob., {:.2f}s)'.format(self._op2, self._op2_prob * 100, self._op2_delay) if self._op2 is not None else 'No digit detected.'
    ...
    def _do_ncs_initialize(self):
        ...
        self._net_processor = MnistProcessor(graph_filename, self._core)
    ...
    def _do_ncs_infer(self, operand, img_label=None):
        ...
        if len(digits) > 0:
            ...
            delay = time.time()-t
            print("value: " + str(value) + " probability: " + str(probability) + " time: " + str(delay))
            return value, probability, delay
        else:
            return None, None
    ...
    def submit(self):
        ...
        self._op1, self._op1_prob, self._op1_delay = self._do_ncs_infer(self._operand1)
        self._op2, self._op2_prob, self._op2_delay = self._do_ncs_infer(self._operand2)
